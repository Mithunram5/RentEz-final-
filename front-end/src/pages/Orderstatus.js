  ...
  const renderButtonsByStatus = () => {
    switch (order.status) {
      case 'Pending':
        return (
          <>
            <button className="accept" onClick={() => handleAction('Payment')}>Accept</button>
      ...
      case 'Payment':
        return (
          <>
            <button className="accept" onClick={() => handleAction('Shipped')}>Paid</button>
            <button className="reject" onClick={() => handleAction('Rejected')}>Reject</button>
          </>
        );
...
const Orders = () => {
  ...
  const applyFilters = () => {
    const filtered = orders.filter(order => {
      ...
      const isWithinDateRange = !filter.date ||
        (filterDate >= fromDate && filterDate <= endDate);
  ...
  const fetchOrders = async () => {
    try {
      ...
      setOrders(Array.isArray(ordersWithImages) ? ordersWithImages : []);
      setFilteredOrders(Array.isArray(ordersWithImages) ? ordersWithImages : []);
    } catch (error) {
